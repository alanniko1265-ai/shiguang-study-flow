...
- **可选监督模式**：空闲阈值可选 10 秒至 5 分钟，暂停前显示 5 秒倒计时；空闲、锁屏或休眠时自动暂停，重新操作后自动继续
- **可靠状态提示**：原生注册 Windows 通知来源，暂停和继续时同时使用右下角通知、任务栏闪烁、应用内提示和计时器状态条
- **系统托盘**：无需打开主窗口即可暂停、继续或完成当前计时
- **安全关机恢复**：计时中每秒强制落盘安全快照，正常退出或 Windows 关机时再次保存；下次启动从已暂停状态恢复，不计入关机时间
...
│       ├── systemIdle.ts       # Windows 系统空闲状态接口
│       ├── notifications.ts    # 系统通知权限与发送接口
│       ├── shutdown.ts         # 关机安全快照接口
...
│       ├── shutdown.rs         # 强制落盘与退出事件恢复快照
│       ├── system_idle.rs      # Windows 空闲与休眠检测
│       ├── system_notifications.rs # Windows 原生通知注册与发送
│       └── tray.rs             # 系统托盘与计时快捷操作
│
├── dist/                       # 前端构建产物
├── release/                    # NSIS 安装包
...
从旧版本升级时，应用会自动检测 `localStorage` 中的 v1 数据，补齐版本号、设备 ID 和时间戳后迁移至 SQLite。0.5.0 会为旧数据库安全增加监督空闲阈值和被扣除的空闲时长字段，并保留全部旧数据。

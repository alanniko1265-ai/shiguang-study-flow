...
- **自定义分类**：预设 4 个默认分类，支持新增自定义分类并选择颜色
...
从旧版本升级时，应用会自动检测 `localStorage` 中的 v1 数据，补齐版本号、设备 ID 和时间戳后迁移至 SQLite v2 表。迁移在本地事务中完成，旧数据不会删除。

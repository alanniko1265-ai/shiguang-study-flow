...
- **可选监督模式**：Windows 全局连续 10 秒没有键盘或鼠标操作时自动暂停，重新操作后自动继续；两种变化都会显示右下角系统通知和应用内提示
...
│       └── system_idle.rs      # Windows 全局键盘/鼠标空闲检测
│
├── dist/                       # 前端构建产物
├── release/                    # NSIS 安装包
...
从旧版本升级时，应用会自动检测 `localStorage` 中的 v1 数据，补齐版本号、设备 ID 和时间戳后迁移至 SQLite。0.4.x 会为旧数据库安全增加分类归档和监督模式设置字段，旧数据不会删除。
